...
The oneAI node is offered in the node panel of every n8n instance. Search for **oneAI** and install it from there.
...
**57 operations across 10 resources.** The tables below are derived from `nodes/OneAi/modes.ts`, the same file the node's **Resource** and **Operation** dropdowns are built from, and they follow the order the dropdown shows. Operations that exist as source files but are not dispatched are not listed here, because they ship to nobody.
...
Hub artifacts — markdown, PDFs, distilled documents — inside spaces.

| Operation      | Description                                                          |
| -------------- | -------------------------------------------------------------------- |
| Create         | Create an artifact in a space, optionally from a chat message         |
| Delete         | Delete an artifact from a space                                       |
| Export PDF     | Export an artifact as a PDF, into a binary property                   |
...
`Create` and `Delete` were removed in a release after the oneAI endpoints behind them stopped existing — see [What this node does not do](#what-this-node-does-not-do).
...
Operations that **write** a file into a binary property: `AI > Generate Image`, `AI > Edit Image`, `AI > Generate Speech`, `Artifact > Export PDF`, `Dataset > Export CSV`, `Space > Download File`.
...
- **No project creation or deletion.** oneAI instantiates projects from templates and retires them by archiving, and neither is a drop-in replacement for a `Create` or a `Delete` operation, so neither is guessed at.
